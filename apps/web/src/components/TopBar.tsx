...
interface DataStatus { countries: string; cables: string; bases: string; earthquakes?: string; eonet?: string; flights?: string }
...
function CounterBox({ label, value, color }: CounterBoxProps) {
    return (
        <div style={{ display: "flex", gap: 5, alignItems: "center", background: "#071523", border: "1px solid #0f2a44", padding: "4px 8px", borderRadius: 3 }}>
...
function FlightStatus({ flightStreamStatus, flightCount }: FlightStatusProps) {
    ...
    return (
        <div style={{ display: "flex", gap: 5, alignItems: "center", background: "#071523", border: `1px solid ${sseDot.color}33`, padding: "4px 8px", borderRadius: 3 }}>
            <div style={{ width: 5, height: 5, borderRadius: "50%", background: sseDot.color, boxShadow: `0 0 4px ${sseDot.color}` }} />
        ...
        <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
            {Object.entries(status).map(([k, v]) => (
                <div key={k} style={{ display: "flex", gap: 3, alignItems: "center" }}>
                    <div style={{ width: 5, height: 5, borderRadius: "50%", background: v === "ok" ? "#00ff88" : v === "error" ? "#ff4444" : "#f59e0b" }} />
...
export function TopBar({ status, baseCount, cableCount, nationCount, quakeCount }: Props) {
    ...
    return (
        <div style={{ height: 48, background: "#050e1a", borderBottom: "1px solid #0f2a44", display: "flex", alignItems: "center", padding: "0 16px", gap: 10, flexShrink: 0, zIndex: 100 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ fontSize: 10, color: "#3a6a8a", letterSpacing: "0.08em" }}>v3.0 // WEEK 3</span>
            </div>
            ...
            <PanelToggleButton label="⚔ CONFLICT" isOpen={conflictPanelOpen} onToggle={handleConflictToggle} accentColor="#f87171" />
            <PanelToggleButton label="📈 ECONOMIC" isOpen={economicPanelOpen} onToggle={handleEconomicToggle} accentColor="#38bdf8" />
